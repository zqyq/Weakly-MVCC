...
from multiview_detector.datasets.frameDataset1 import frameDataset
from multiview_detector.datasets import Wildtrack, MultiviewX
from multiview_detector.loss.gaussian_mse import GaussianMSE

from multiview_detector.models.cvcs_map_weight import PerspTransDetector_max_cvcs as PerspTransDetector
...
from multiview_detector.trainer_weight import PerspectiveTrainer as PerspectiveTrainer
...
def main(args):
    ...
    isTest = args.mode != 'train'

    train_set = frameDataset(base, dataset_dir=args.dataset_dir, batch_size=args.batch_size, train=True,
                             transform=train_trans, grid_reduce=4, seed=args.seed)

    test_set = frameDataset(base, dataset_dir=args.dataset_dir, batch_size=args.batch_size, train=False,
                            transform=train_trans, grid_reduce=4, seed=args.seed, isTest=isTest)
    ...
    # logging
    logdir = f'logs/{args.dataset}_frame/ll/' + datetime.datetime.today().strftime('%Y-%m-%d_%H-%M-%S') \
    ...
    trainer = PerspectiveTrainer(model, criterion, logdir, denormalize, args.cls_thres, args.alpha, args)

    pretrain = True
    if pretrain:
        # img_pred_name = "/mnt/d/DJ/CF/logs/wildtrack_frame/trans_2d/2025-02-02_05-33-49/models/Ep_164_mae_16.48952877628478_mse_496.92221207536954.pth"
        ...
        #
        # weight_pred_name = "/mnt/d/DJ/CF/ pretrain_model/origion/FeatDist/60-2195.9841.pth"
        # pretrain_model = torch.load(weight_pred_name)
        # pretrain_para = pretrain_model['model']
        #
        # model_para = model.state_dict()
        # useful_para2 = {k: v for k, v in pretrain_para.items() if k in model_para and k.split('.')[0] != 'img_classifier'}
        #
        ...
        if isTest:
            weight_name = args.ckpt
        else:
            # weight_name = "/mnt/d/DJ/CF/ pretrain_model/origion/FeatDist/60-2195.9841.pth"
            weight_name = "/mnt/d/DJ/CF/ pretrain_model/origion/Labels_Reproduce_homography2_supervision_counting_FeaDistAtt_rightOutputDim_GD/71-8954.4453-MAE18.90.pth"
        ...
        if isTest:
            ...
            useful_para = {k: v for k, v in pretrain_para.items() if k in model_para and k.split('.')[0] not in ['match_CNN']}
            # useful_para = {k: v for k, v in pretrain_para.items() if k in model_para}
        model_para.update(useful_para)
        model.load_state_dict(model_para)
    ...
    if isTest:
        ...
        for epoch in tqdm.tqdm(range(1, args.epochs + 1)):
            ...
            test_loss, test_prec, mae = trainer.test(test_loader, os.path.join(logdir, 'test.txt'),
                                                      train_set.gt_fpath, True, epoch=epoch)
            ...
            if best_mae > mae or epoch % 10 == 0:
                ...
                torch.save(checkpoint, os.path.join(logdir, 'models/epoch' + str(epoch) + "_mae_" + str(mae) + '.pth'))
...
if __name__ == '__main__':
    ...
    # parser.add_argument('--ckpt', type=str, default='logs/wildtrack_frame/ll/2025-05-28_10-06-58/models/epoch1_mae_17.01692220661673.pth')
    parser.add_argument('--density_scale', type=int, default=100)
    parser.add_argument('--augmentation', action='store_true')
    parser.add_argument('--multi_view_train', action='store_true', default=True)
    ...
    # 0.001
    parser.add_argument('--lr', type=float, default=1e-6, metavar='LR', help='learning rate (default: 0.1)')
