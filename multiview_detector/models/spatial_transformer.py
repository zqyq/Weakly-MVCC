...
def spatial_transformation_layer(paras, x, ph=1):
    ...
    # paras.view_size = view_size
    # paras.patch_num = patch_num
    # paras.cropped_size = cropped_size
    # projection result
    output = _transform(paras, x, ph)

    # batch_size = output.shape[0].value
    # height = output.shape[1].value
    # width = output.shape[2].value
    # num_channels = output.shape[3].value
    #
    # # cropping:
    # #_, _, input_wld_map_paras = inputs
    # h_actual = tf.maximum(actual_size[0], cropped_size[0])
    # w_actual = tf.maximum(actual_size[1], cropped_size[1])
    #
    # output_cropped = output[:, :h_actual, :w_actual, :]
    #
    # size = [batch_size, cropped_size[0], cropped_size[1], num_channels]
...
def _transform(paras, inputs, ph):
    ...
    transformed_image_all = torch.zeros([1, patch_num, output_height, output_width, num_channels]).to(input_feature.device)
    for i in range(batch_size):
        ...
        for p in range(patch_num):
            ...
            transformed_grid = _create_transformed_grid(indices_grid.to(input_feature.device),
                                                             input_camera_paras[batch_id, 0].to(input_feature.device),
                                                             input_wld_map_paras_i.to(input_feature.device),
                                                             [height, width],
                                                        ph).to(input_feature.device)
...
def _interpolate(image, x, y, output_size):
    ...
    pixels_batch = torch.arange(0, batch_size, dtype=torch.float32) * flat_image_dimensions
...
def _projectPoints(
    ...
    r2 = (torch.tensor([1]).to(wld_coords_transed.device) - torch.cos(theta)) * torch.matmul(rvec, torch.transpose(rvec, 0, 1))
...
def _create_transformed_grid(
    ...
    r, a, b, h_actual, w_actual, d_mean, w_min, h_min, w_max, h_max = input_wld_map_paras[0].to(indices_grid.device), input_wld_map_paras[1].to(indices_grid.device), \
