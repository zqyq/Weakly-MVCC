...
class PerspTransDetector_max_cvcs(nn.Module):
    def __init__(self, dataset, arch='vgg11'):  # vgg11 resnet18
        ...
        self.corr_encoder = nn.Sequential(nn.Conv2d(1, 64, 3, padding='same'), nn.ReLU(),
                                    ...
                                    nn.Conv2d(256, 256, 3, padding='same'), nn.ReLU(),
                                    nn.Conv2d(256, 256, 3, padding='same'), nn.ReLU(),
                                    nn.MaxPool2d(2)).to(self.device[0])

        self.weight_pred = nn.Sequential(nn.Conv2d(3948, 64, 3, padding='same'), nn.ReLU(),
    ...
    def forward(self, imgs, distmap, train=True):
        ...
        w = []
        corr_feature_ = corr_feature.permute(0, 2, 3, 1)
        for i in range(N):
            feature_i = corr_feature_[i:i+1]
            feature_i = feature_i.repeat(N-1, 1, 1, 1)

            feature_others = torch.cat([corr_feature_[:i], corr_feature_[i+1:]], dim=0)
        ...
        # weight = 1 / (1 + w)
        distmap2 = nn.MaxPool2d(2)(distmap[0].float())
        dist_feature = self.distence_extractor(distmap2.to(self.device[0]))
        # dist_feature *= corr_feature.max()
        extra_feature = torch.cat([corr_feature, dist_feature], dim=1)
