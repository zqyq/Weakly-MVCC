...
def spatial_transformation_layer(paras, x, ph=1):
    output = _transform(paras, x, ph)

    # (B, P, H, W, C) -> (B, P, C, H, W)
    output = output.permute((0, 1, 4, 2, 3))
    return output  # output_cropped2


def _transform(paras, inputs, ph):
    batch_size0 = paras[0]
    view_size = paras[1]
    patch_num = paras[2]
    ...
    # due to the shape of origin code is (B, H, W, C) rather than (B, C, H, W)
    input_feature = input_feature.permute(dims=(0, 2, 3, 1))
    ...
    batch_size = input_feature.shape[0]  # actually, equals b*v, but the v is one due to the handle of unit v
    ...
    # output_height, output_width = input_wld_map_paras[0][4], input_wld_map_paras[0][5]
    # output_size = tf.cast([output_height, output_width], dtype='int32')
    ...
    for i in range(batch_size):
        ...
        # set indices_grid:
        batch_id = i  # actually, equals b*v
        input_wld_map_paras_i = input_wld_map_paras[batch_id, :]

        transformed_image_p = torch.zeros([1, output_height, output_width, num_channels]).to(device)
        for p in range(patch_num):
            hw = input_hw_random[batch_id, :, p]

            indices_grid = _meshgrid(output_height, output_width, hw).to(device)
            ...
            # (1, (x, y, z), len(x))
            # indices_grid = torch.tile(indices_grid, torch.stack([1]))
            indices_grid = torch.reshape(indices_grid, (1, 3, -1))

            transformed_grid = _create_transformed_grid(indices_grid.to(device),
                                                        input_camera_paras[batch_id, 0].to(device),
                                                        input_wld_map_paras_i.to(device),
                                                        [height, width],
                                                        ph).to(device)
    ...
    transformed_image_all = transformed_image_all[1:].to(device)

    return transformed_image_all


def _repeat(x, num_repeats):
    ones = torch.ones((1, num_repeats), dtype=torch.int32)
...
def _interpolate(image, x, y, output_size):
    ...
    zero = torch.zeros([1])
    device = image.device
    x0 = torch.clamp(x0, 0, max_x).to(device)
    x1 = torch.clamp(x1, 0, max_x).to(device)
    y0 = torch.clamp(y0, 0, max_y).to(device)
    y1 = torch.clamp(y1, 0, max_y).to(device)

    flat_image_dimensions = width * height
    # pixels_batch = torch.range(0, batch_size-1) * flat_image_dimensions
    pixels_batch = torch.arange(0, batch_size, dtype=torch.int32) * flat_image_dimensions
    flat_output_dimensions = output_height * output_width
    base = _repeat(pixels_batch, flat_output_dimensions).to(device)
    ...
    # flat_image = flat_image.clone().detach()
...
def _meshgrid(height, width, offset):
    ...
    # x_linspace = torch.linspace(0, width, width)
    # y_linspace = torch.linspace(0, height, height)
    # the order of the torch is contrary to the tensorflow
...
def _projectPoints(
        ...
        fx, fy, u, v,
        distCoeffs):
    device = wld_coords_transed.device
    # wld
    ...
    theta = torch.norm(rvec).to(device)
    rvec = torch.div(rvec, theta)
    rx, ry, rz = rvec[0], rvec[1], rvec[2]
    skew = torch.tensor([[0, -rz, ry],
                         [rz, 0, -rx],
                         [-ry, rx, 0]]).to(device)  # , dtype='float32')
    rvec = torch.unsqueeze(rvec, dim=1)
    r1 = torch.cos(theta) * torch.eye(3).to(device)
    r2 = (torch.tensor([1]).to(device) - torch.cos(theta)) * torch.matmul(rvec,
    ...
    # step 2:
    Xu0 = torch.div(xc, zc + 1.0 / 19200.0)  # avoid zero
    Yu0 = torch.div(yc, zc + 1.0 / 10800.0)
...
def _create_transformed_grid(
    ...
    r, a, b, h_actual, w_actual, d_mean, w_min, h_min, w_max, h_max = input_wld_map_paras[0].to(indices_grid.device), \
        ...
        input_wld_map_paras[3].to(indices_grid.device), input_wld_map_paras[4].to(indices_grid.device), \
        input_wld_map_paras[5].to(indices_grid.device), \
        input_wld_map_paras[6].to(indices_grid.device), input_wld_map_paras[7].to(indices_grid.device), \
    ...
    wld_coords_transed_d = indices_grid[:, 2:, :] * d_mean * ph
    ...
    transformed_grid_w = (transformed_grid[:, 0:1, :] * 2.0) / 1920 - 1.0  # ori resolution 1920*1080
