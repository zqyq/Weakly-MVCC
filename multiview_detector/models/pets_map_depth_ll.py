...
class PerspTransDetector_max_cvcs(nn.Module):
    def __init__(self, dataset, arch='vgg11', bottleneck_dim=128):  # vgg11 resnet18
        super().__init__()
        ...
        self.output_size = [72, 96]
        self.imgsize = self.output_size
        ...
        if bottleneck_dim:
            self.bottleneck = nn.Sequential(nn.Conv2d(out_channel, bottleneck_dim, 1), nn.Dropout2d(0.0)).to(
                self.device[0])
    ...
    def forward(self, imgs, distmap, train=True, imgs_single_C=None):
        ...
        imgs_single_C = imgs_single_C.view(-1, 1, H, W).float().to(self.device[0])
        corr_feature0 = self.corr_encoder(imgs_single_C.to(self.device[0]))
        ...
        # img_res = F.interpolate(img_res, size=weight.shape[-2:], mode='bilinear', align_corners=True)  / 100
        img_res /= 100
