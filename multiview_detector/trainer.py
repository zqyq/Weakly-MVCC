...
class PerspectiveTrainer(BaseTrainer):
    ...
    def train(self, epoch, data_loader, optimizer, log_interval=100, cyclic_scheduler=None):
        ...
        for batch_idx, (data, imgs_gt, map_gt, homography, M_gt, depthMap) in enumerate(
                data_loader):
            # if (batch_idx + 1) % 60 == 0:
            #     continue
            optimizer.zero_grad()
            ...
            if (batch_idx + 1) % 1500 == 0 or (len(data_loader) < 200 and (batch_idx + 1) % 20 == 0):
                fig = plt.figure()
                subplt0 = fig.add_subplot(121, title="output")
                subplt1 = fig.add_subplot(122, title="target")
                subplt0.imshow(dist_score[0, 0, :, :].cpu().detach().numpy())
                subplt1.imshow(dist_score[1, 0, :, :].cpu().detach().numpy())
                plt.savefig(os.path.join(self.logdir, f'C01_{str(batch_idx + 1)}.jpg'))
                plt.close(fig)
            ...
            if (batch_idx + 1) % log_interval == 0:
                ...
                print('Train Epoch: {}, Batch:{}, Loss: {:.6f}, curLoss: {:.6f}, dist_score: {:.6f}, lr: {:.6f}, '
                      'MAE: {:.3f}, NAE: {:.3f}, RMSE: {:.3f} Time: {:.1f} (f{:.3f}+b{:.3f}), maxima: {:.3f}'.format(
                    epoch, (batch_idx + 1), losses / (batch_idx + 1), loss, dist_score.max(), current_lr,  mae.item() / (batch_idx + 1),
    ...
    def test(self, data_loader, res_fpath=None, gt_fpath=None, visualize=False):
        ...
        # for batch_idx, (data, map_gt, imgs_gt, frame) in enumerate(data_loader):
        # img_views, single_view_dmaps, camera_paras, wld_map_paras, hw_random, GP_density_map
        for batch_idx, (data, imgs_gt, map_gt, homography, weight, depthMap) in enumerate(
                data_loader):
            ...
            map_gt = torch.reshape(map_gt, (1, -1)).to(map_res.device)
            gt = torch.sum(map_gt, dim=-1, keepdim=True)
            loss = abs(map_res - gt)
        ...
        return losses / len(data_loader), precision_s.avg * 100, 0
