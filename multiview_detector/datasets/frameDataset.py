...
class frameDataset(VisionDataset):
    def __init__(self, base, dataset_dir=None, batch_size=1, train=True, transform=ToTensor(), target_transform=ToTensor(),
                 reID=False, grid_reduce=4, img_reduce=4, train_ratio=0.9, force_download=True, seed=14):
    ...
    def __getitem__(self, index):
        ...
        # select views first:
        frame_0 = frame_j  # frame_name_list[0]
        label_path0 = os.path.join(self.label_file_path, scene_i, frame_0, 'json_paras/')
        # label_path0 = os.path.join(self.depth_file_path, scene_i)
        ...
        homography = []
        for v1 in label_path_list_sampling:
        ...
        imgh_range = range(0, 360 - 192 + 1)  # 192 大小的块, 或者 192 / 2
        imgw_range = range(0, 640 - 192 + 1)
        ...
        for p in label_path_list_sampling:
            ...
            # depthMap_i = (depthMap_i.max() - depthMap_i) / 1000
            ...
            for x0, x1 in coords_2d:
                cx = int(x0 * 160)
                cy = int(x1 * 90)
                map_counting(single_view_dmaps_i, (cx, cy), sigma=1)

                ci = int(x0 * 640)
                cj = int(x1 * 360)
                if (ci >= imgw_random[0] and ci <= imgw_random[0]+192 and cj >= imgh_random[0] and cj <= imgh_random[0]+192):  # ranking
                    mask1_dmaps_i[int(cj / 4), int(ci / 4)] = 1
                if (ci >= imgw_random[1] and ci <= imgw_random[1]+192 and cj >= imgh_random[1] and cj <= imgh_random[1]+192):
                    # map_counting(mask1_dmaps_i, (ci, cj), sigma=3)
                    mask2_dmaps_i[int(cj/4), int(ci/4)] = 1
                if (ci >= imgw_random[0]+32 and ci <= imgw_random[0]+160 and cj >= imgh_random[0]+32 and cj <= imgh_random[0]+160):
                    # map_counting(mask2_dmaps_i, (ci, cj), sigma=3)
                    mask3_dmaps_i[int(cj/4), int(ci/4)] = 1
                if (ci >= imgw_random[1]+32 and ci <= imgw_random[1]+160 and cj >= imgh_random[1]+32 and cj <= imgh_random[1]+160):
                    mask4_dmaps_i[int(cj/4), int(ci/4)] = 1
                # tmp[cy, cx] = 1
        ...
        mask1 = np.zeros((1, 3, 360, 640))
        mask2 = np.zeros((1, 3, 360, 640))
        mask3 = np.zeros((1, 3, 360, 640))
        mask4 = np.zeros((1, 3, 360, 640))


        mask1[:, :, imgh_random[0]:imgh_random[0]+192, imgw_random[0]:imgw_random[0]+192] = 1  #  192 contains 128, i.e. mask1 contains mask3, mask2 contains mask4
        mask1_img = img_views.copy()
        # mask1_img[:, :, imgh_random:imgh_random+192, imgw_random:imgw_random+192] = mask1
        mask1_img = np.multiply(mask1_img, mask1)

        mask2[:, :, imgh_random[1]:imgh_random[1] + 192, imgw_random[1]:imgw_random[1] + 192] = 1
        mask2_img = img_views.copy()
        # mask1_img[:, :, imgh_random:imgh_random+192, imgw_random:imgw_random+192] = mask1
        mask2_img = np.multiply(mask2_img, mask2)

        mask3_img = img_views.copy()
        # mask2_img[:, :, imgh_random+32:imgh_random+160, imgw_random+32:imgw_random+160] = mask2
        mask3[:, :,  imgh_random[0]+32:imgh_random[0]+160, imgw_random[0]+32:imgw_random[0]+160] = 1
        mask3_img = np.multiply(mask3_img, mask3)


        mask4_img = img_views.copy()
        # mask2_img[:, :, imgh_random+32:imgh_random+160, imgw_random+32:imgw_random+160] = mask2
        mask4[:, :, imgh_random[1] + 32:imgh_random[1] + 160, imgw_random[1] + 32:imgw_random[1] + 160] = 1
        mask4_img = np.multiply(mask4_img, mask4)
        imgs = np.concatenate([img_views, mask1_img, mask2_img, mask3_img, mask4_img], axis=0).reshape(5, self.num_cam, 3, img_views.shape[-2],
                                                                                 img_views.shape[-1])
        ...
        # plt.show()
        #
        # plt.imshow(single_view_dmaps[0][0])
        # plt.show()

        # print(scene_i + " " + frame_j + " " + str(label_path_list_sampling))
        # return img_views, single_view_dmaps, GP_density_map
        # return img_views, single_view_dmaps, GP_density_map[0,0], homography
        return imgs, single_view_dmaps, GP_density_map   # 单视角
...
def test():
    ...
    # path = "/mnt/d/common/Datasets/LCVCS/depth_map/val/scene_50/5.raw"
    # depthMap = np.fromfile(path, dtype=np.float32)
    # depthMap = depthMap.reshape(1080, 1920)

    dataset = frameDataset(MultiviewX(os.path.expanduser('/mnt/d/data/MultiviewX')), train=False, batch_size=2)
    # test projection
    world_grid_maps = []
    ...
    import matplotlib.pyplot as plt
    img_views, single_view_dmaps, GP_density_map, homography = dataset.__getitem__(100)
    pass
