...
class Pets_ll(VisionDataset):
    ...
    def get_img_view_path(self, train=True):
        if train:
            view1 = ['S1L3/Time_14-17/View_001', 'S1L3/Time_14-33/View_001',
                                  'S2L2/View_001', 'S2L3/Time_14-41/View_001']
            view2 = ['S1L3/Time_14-17/View_002', 'S1L3/Time_14-33/View_002',
                                  'S2L2/View_002', 'S2L3/Time_14-41/View_002']
            view3 = ['S1L3/Time_14-17/View_003', 'S1L3/Time_14-33/View_003',
                                  'S2L2/View_003', 'S2L3/Time_14-41/View_003']
        else:
            view1 = ['S1L1/Time_13-57/View_001', 'S1L1/Time_13-59/View_001',
                                  'S1L2/Time_14-06/View_001', 'S1L2/Time_14-31/View_001']
            view2 = ['S1L1/Time_13-57/View_002', 'S1L1/Time_13-59/View_002',
                     'S1L2/Time_14-06/View_002', 'S1L2/Time_14-31/View_002']
            view3 = ['S1L1/Time_13-57/View_003', 'S1L1/Time_13-59/View_003',
                     'S1L2/Time_14-06/View_003', 'S1L2/Time_14-31/View_003']
    ...
    def get_img_fpath(self, train=True):
        # (cam, frame)
        all_view_path = self.get_img_view_path(train)
        viewimg_fpaths = {view: {} for view in range(1, 4)}
        for view_i, all_view_i_path in enumerate(all_view_path):
            last_patch_len = 0
            for view_i_path in all_view_i_path:
                is_S2L2 = view_i_path.startswith('S2L2')
                frame_path = os.path.join(self.root, 'image_frames', 'Train' if train else 'Test', view_i_path)
                for fname in os.listdir(frame_path):
                    frame = int(fname.split('_')[1].split('.')[0])
                    frame += last_patch_len
                    if is_S2L2:
                        frame -= 3
                    # if frame in range(636, 1636, 2):
                    viewimg_fpaths[view_i + 1][frame] = os.path.join(frame_path, fname)
                last_patch_len = len(viewimg_fpaths[view_i + 1])
                # print(last_patch_len)
        # for ids, frame_idx in enumerate(sorted(viewimg_fpaths[view_i + 1])):
        #     print(f'{ids}: {frame_idx}')
